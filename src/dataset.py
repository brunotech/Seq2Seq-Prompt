...
class OurInputFeatures(InputFeatures):
    ...
    label_words_list: Optional[List[int]] = None  # Label word mapping (dynamic)
    guid: Optional[Union[str, int, List[int]]] = None
...
def input_example_to_tuple(example):
    if example.text_b is None:
        ...
        return [example.text_a, example.text_b, example.text_c, example.text_d]
...
def tokenize_multipart_input(
        ...
        *xx* represent variables:
        ...
        for part_id, part in enumerate(template_list):
            ...
            if part in special_token_mapping:
                ...
                # Note that label_words_list already has extra space, so do not add more space ahead of it.
                label_id = int(part.split("_")[1])
                label_words = label_words_list[label_id]
                new_tokens += label_words
            elif part[:7] == "labelx_":
                ...
                # prompt tuning
                if use_pt:
                    if not prompt_encoder.init_success:
                        prompt_encoder.add_prompt_tokens(new_tokens)
                    new_tokens = prompt_encoder.get_prompt_tokens(
                        new_tokens_begin, len(new_tokens)
                    )
                    new_tokens_begin += len(new_tokens)

            if (part[:4] == "sent" or part[1:5] == "sent") and part[1:9] != "sentence":
    ...
    # Find mask token
    if prompt and "T5" not in type(tokenizer).__name__:
    ...
    if use_pt:
        prompt_encoder.init_success = True
...
class FewShotDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if args.prompt:
            ...
            self.label_to_words = eval(args.mapping)

            for key in self.label_to_words:
                # For RoBERTa/BART/T5, tokenization also considers space, so we use space+word as label words.
                # Make sure space+word is in the vocabulary
                self.label_to_words[key] = tokenizer.convert_tokens_to_ids(
                    tokenizer.tokenize(self.label_to_words[key])
                )
                if self.label_to_words[key][0] == 3:
                    self.label_to_words[key] = self.label_to_words[key][1:]
                logger.info(
                    "Label {} to words {} ({})".format(
                        key,
                        tokenizer.convert_ids_to_tokens(self.label_to_words[key]),
                        self.label_to_words[key],
                    )
                )

            if self.task_name == "copa" and "man" in args.tag:
                if mode == "train":
                    _, self.label_words_list = self.processor.get_train_examples(
                        args.data_dir, manual=True
                    )
                elif mode == "dev":
                    _, self.label_words_list = self.processor.get_dev_examples(
                        args.data_dir, manual=True
                    )
                elif mode == "test":
                    _, self.label_words_list = self.processor.get_test_examples(
                        args.data_dir, manual=True
                    )
            elif self.task_name == "record":
                fine_tuning = "fine-tuning" in args.tag
                if mode == "train":
                    _, self.label_words_list = self.processor.get_train_examples(
                        args.data_dir, fine_tuning=fine_tuning
                    )
                elif mode == "dev":
                    _, self.label_words_list = self.processor.get_dev_examples(
                        args.data_dir, fine_tuning=fine_tuning
                ...
                self.label_words_list = {
                    example.guid: example.text_b for example in examples
                }
            else:
                if len(self.label_list) > 1:
                    self.label_words_list = [
                        self.label_to_words[label] for label in self.label_list
                    ]
                else:
                    # Regression task
                    # '0' represents low polarity and '1' represents high polarity.
                    self.label_words_list = [
                        self.label_to_words[label] for label in ["0", "1"]
                    ]
        else:
            self.label_to_words = None
        ...
        with FileLock(lock_path):
            ...
            if os.path.exists(cached_features_file) and not args.overwrite_cache:
                ...
                if self.task_name == "copa" and "man" in args.tag:
                    self.support_examples, _ = self.processor.get_train_examples(
                        args.data_dir, manual=True
                    )

                    if mode == "dev":
                        self.query_examples, _ = self.processor.get_dev_examples(
                            args.data_dir, manual=True
                        )
                    elif mode == "test":
                        self.query_examples, _ = self.processor.get_test_examples(
                            args.data_dir, manual=True
                        )
                    else:
            ...
            for query_idx, context_indices, bootstrap_idx in self.example_idx:
                ...
                self.features.append(
                    self.convert_fn(
                        ...
                        prompt=args.prompt,
                        template=template,
                        label_words_list=self.label_words_list,
    ...
    def convert_fn(
        ...
        if not use_demo:
            ...
            inputs = tokenize_multipart_input(
                ...
                prompt=prompt,
                template=template,
                label_words_list=label_words_list,
                first_sent_limit=self.args.first_sent_limit,
                other_sent_limit=self.args.other_sent_limit,
                use_pt=use_pt,
            )
            features = OurInputFeatures(
                **inputs, label=example_label, guid=example.guid
            )
